...
Afetrwards I thought which one coul be the aim of my analysis and, watching this year MSI (Mid Season Invitational), it came to my mind the possibility to analyze how much impact have first Blood, first Dragon, first Baron Nashor and game side (blue or red) on the victory.
...
So I used **select** to maintain only the columns I wanted, furthermore I create a new column named *Result* that has the same value of *bResult* and it symbolizes the winning team (if blue team won it has value 1, instead if red team won the value is 0).
...
Once that I cleaned the *mathinfo* table and put it in the new one called *interestingData*, it was time to tidy the other tables to let them fit in the base one.
...
To do this I use a **for** to count how many times the team that drawned the First Blood also won the game.
The result is expressed both in th absolute value and percentage.
...
The operations are the same of the ones for the First Blood.
...
That was easily predictable because the Baron is a monster that gives an iportant buff to the team that kills it and it spawns later in the game (the first one spawn at minute 20).
...
I'm going to expect a bigger impact of the blue side because there has always been said that the team that plays from the blue side is benefites, even if the the advantage isn't huge.
...
Then I create the new table *champions* and I add the column with the names.
...
This value means that the champions in the new table have been seen for more than 254 games per year, which is a insignificant number of games given that the total number of game per year is 2540.
The new table has only 64 lines, which means that only 64 champions reach that value of presence, against the 140 lines of the originaty *champions* table.
...
In conclusion, these analysis shows how the professional League of Legends has some "rules" all his and how many things have to be studied in this world.
...
The team that takes down the First Baron has statistically an advantage that will lead them to win the game in the 80% of the times.
...
Moreover the result point out how difficult is for the proplayer to change their habits and learn some new champions to adapt the gameplay to the changes of the meta.
